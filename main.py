...
def addtask_command(message):
    """
    👉используя эту функцию, вы можете сохранить новый проект!👈
    """
...
def skill_handler(message):
    """
    👀используй, чтобы привязать к проекту навыки, которые в нем использовали👀
    """
    user_id = message.from_user.id
...
def get_projects(message):
    """
    🧐используй, чтобы увидить все сохраненные проекты🧐
    """
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
...
def delete_handler(message):
    """
    ❌используй, чтобы полностью удалить сохраненный проект❌
    """
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
...
def update_project(message):
    """
    ✍️используй, чтобы изменить данные сохраненного проекта✍️
    """
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)
    if projects:
